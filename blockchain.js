...
class Block {
  ...
  calculateHash(nonce) {
    // Your code here
    const transactionString = this.transactions.map(text => text.signature).join('');
    this.nonce = nonce;
    this.hash = createHash('sha512').update(this.previousHash + transactionString + nonce).digest('hex');
  }
}
...
class Blockchain {
  ...
  constructor() {
    // Your code here
    // "new Block([],null)" is the genesis block
